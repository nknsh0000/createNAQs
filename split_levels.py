...
def return_level(context, question, bounds):
	...
	cws = funcs.document_normalization(context)
	qws = funcs.document_normalization(question)

	cwvecs = []
	for cw in cws:
		cwvec = funcs.return_vec(cw)
		if not cwvec == -1:
			cwvecs.append(cwvec)
	maxcos = 0
	coss = []
	for qw in qws:
		qwvec = funcs.return_vec(qw)
		if qwvec == -1:
			continue
		for cwvec in cwvecs:
			cos = funcs.cossim(qwvec, cwvec)
